...
Clamp do
  ...
  subcommand "create_users", "Creates system users from the iam users in given group" do
      ...
      iam_users.each do |iam_user|
        ...
        if ssh_public_keys.size > 0
          ...
          if Regexp.new(r) =~ iam_user
            ...
            $stderr.puts "Skipping user '#{iam_user}' as unable to match pattern '#{iam_user_pattern}'"
          end
        else
